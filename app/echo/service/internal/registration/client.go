...
type Client struct {
	...
	client         adminV1.ModuleRegistrationServiceClient
...
func NewClient(logger log.Logger, config *Config) (*Client, error) {
	l := log.NewHelper(log.With(logger, "module", "registration/echo-service"))
	...
	return &Client{
		...
		config:   config,
		conn:     conn,
		client:   adminV1.NewModuleRegistrationServiceClient(conn),
...
func (c *Client) Register(ctx context.Context) error {
	...
	req := &adminV1.RegisterModuleRequest{
...
func (c *Client) sendHeartbeat(ctx context.Context) error {
	req := &adminV1.HeartbeatRequest{
		ModuleId: c.config.ModuleID,
		Health:   adminV1.ModuleHealth_MODULE_HEALTH_HEALTHY,
		Message:  "Echo service is healthy",
	}
...
func (c *Client) Unregister(ctx context.Context) error {
	...
	req := &adminV1.UnregisterModuleRequest{
		ModuleId:  c.config.ModuleID,
		AuthToken: c.config.AuthToken,
	}
