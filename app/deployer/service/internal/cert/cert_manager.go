...
// CertManager manages TLS certificates for the deployer service
...
func NewCertManager(ctx *bootstrap.Context) (*CertManager, error) {
	l := ctx.NewLoggerHelper("deployer/cert")
	...
	serverCertPath := os.Getenv("DEPLOYER_SERVER_CERT_PATH")
	if serverCertPath == "" {
		serverCertPath = "/app/certs/server/server.crt"
	}
	serverKeyPath := os.Getenv("DEPLOYER_SERVER_KEY_PATH")
	if serverKeyPath == "" {
	...
	if err := cm.validateCertFiles(); err != nil {
		...
		// Don't fail - we'll handle missing certs gracefully
	}
