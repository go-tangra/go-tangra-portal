...
// CertManager manages TLS certificates for the Paperless service
...
func NewCertManager(ctx *bootstrap.Context) (*CertManager, error) {
	l := ctx.NewLoggerHelper("paperless/cert")
	...
	serverCertPath := os.Getenv("PAPERLESS_SERVER_CERT_PATH")
	if serverCertPath == "" {
		serverCertPath = "/app/certs/server/server.crt"
	}
	serverKeyPath := os.Getenv("PAPERLESS_SERVER_KEY_PATH")
	if serverKeyPath == "" {
