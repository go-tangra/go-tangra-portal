...
func (r *RoleRepo) CreateWithTx(ctx context.Context, tx *ent.Tx, data *userV1.Role) (dto *userV1.Role, err error) {
	...
	if isTemplate {
		...
		templateVersion = trans.Ptr(int32(1)) // Templates start at version 1
	} else {
		// Non-template roles use role ID as version to ensure unique constraint satisfaction
		templateVersion = trans.Ptr(int32(ret.ID))
	}
