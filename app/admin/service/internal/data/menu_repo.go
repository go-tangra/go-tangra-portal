...
func (r *MenuRepo) Update(ctx context.Context, req *permissionV1.UpdateMenuRequest) error {
	...
	var hasMetaInMask bool
	if req.UpdateMask != nil {
		for _, v := range req.UpdateMask.GetPaths() {
			if v == "meta" {
				hasMetaInMask = true
			} else if strings.HasPrefix(v, "meta.") {
	...
	err := r.repository.UpdateX(ctx, builder, req.Data, req.GetUpdateMask(),
		func(dto *permissionV1.Menu) {
			...
			if req.Data.Meta != nil {
				r.updateMetaField(builder, req.Data.Meta, metaPaths, hasMetaInMask)
			}
		},
		func(s *sql.Selector) {
...
func (r *MenuRepo) updateMetaField(builder *ent.MenuUpdate, meta *permissionV1.MenuMeta, metaPaths []string, updateEntireMeta bool) {
	// If "meta" is in the update mask (not specific sub-paths), update the entire meta object
	if updateEntireMeta && len(metaPaths) == 0 {
		builder.SetMeta(meta)
		return
	}

	// Otherwise, update only specific fields within meta
	// 删除空值
...
func (r *MenuRepo) Delete(ctx context.Context, req *permissionV1.DeleteMenuRequest) error {
	...
	ids, err := QueryAllChildrenIds(ctx, r.entClient, "sys_menus", req.GetId())
	if err != nil {
	...
	// Convert []uint32 to []any for sql.In()
...
// GetMenuByName retrieves a menu by its unique name.
func (r *MenuRepo) GetMenuByName(ctx context.Context, name string) (*ent.Menu, error) {
	entity, err := r.entClient.Client().Menu.Query().
		Where(menu.NameEQ(name)).
		Only(ctx)
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, nil // Menu not found is not an error
		}
		r.log.Errorf("query menu by name failed: %s", err.Error())
		return nil, permissionV1.ErrorInternalServerError("query menu by name failed")
	}
	return entity, nil
}

// FindMenusByNamePrefix finds all menus whose name starts with the given prefix.
func (r *MenuRepo) FindMenusByNamePrefix(ctx context.Context, prefix string) ([]*ent.Menu, error) {
	entities, err := r.entClient.Client().Menu.Query().
		Where(menu.NameHasPrefix(prefix)).
		All(ctx)
	if err != nil {
		r.log.Errorf("query menus by prefix failed: %s", err.Error())
		return nil, permissionV1.ErrorInternalServerError("query menus by prefix failed")
	}
	return entities, nil
}

// DeleteByID deletes a menu by its database ID.
func (r *MenuRepo) DeleteByID(ctx context.Context, id uint32) error {
	err := r.entClient.Client().Menu.DeleteOneID(id).Exec(ctx)
	if err != nil {
		r.log.Errorf("delete menu by id failed: %s", err.Error())
		return permissionV1.ErrorInternalServerError("delete menu by id failed")
	}
	return nil
}
