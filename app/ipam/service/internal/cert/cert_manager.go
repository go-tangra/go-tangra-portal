...
// CertManager manages TLS certificates for the IPAM service
...
func NewCertManager(ctx *bootstrap.Context) (*CertManager, error) {
	l := ctx.NewLoggerHelper("ipam/cert")
	...
	serverCertPath := os.Getenv("IPAM_SERVER_CERT_PATH")
	if serverCertPath == "" {
		serverCertPath = "/app/certs/server/server.crt"
	}
	serverKeyPath := os.Getenv("IPAM_SERVER_KEY_PATH")
	if serverKeyPath == "" {
