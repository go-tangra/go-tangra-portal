...
// Reserved event types are the internal SSE/system framing types (reset|bye)
// that modules must not publish; every other lowercase dotted type (e.g.
// certificate.issued, warden.share.created) is a valid module event.
var Reserved = map[string]bool{"reset": true, "bye": true}
