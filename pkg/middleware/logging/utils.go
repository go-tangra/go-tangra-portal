...
// isLocalNetworkLocation checks if the location string indicates a local/private network
func isLocalNetworkLocation(s string) bool {
...
// fillGeoLocation fills geographic location information
func fillGeoLocation(clientIp string) (info *auditV1.GeoLocation) {
	...
	// Check if this is a local/private network address
	if isLocalNetworkLocation(result.Country) || isLocalNetworkLocation(result.Province) || isLocalNetworkLocation(result.City) {
